...
else if(isset($_GET['latest_ringtone']))
{
	...
	$query_rec = "SELECT COUNT(*) as num FROM tbl_ringtone
	ORDER BY tbl_ringtone.id";
	...
	ORDER BY tbl_ringtone.id DESC LIMIT $limit,$page_limit";
	...
	{	
		$row['num'] = $total_pages['num'];
		$row['id'] = $data['id'];			 
		$row['ringtone'] = $file_path.'images/ringtone/'.$data['ringtone'];
		$row['ringtone_tag'] = $data['ringtone_tag'];
		$row['total_views'] = $data['total_views']; 
		$row['total_rate'] = $data['total_rate'];
		$row['rate_avg'] = $data['rate_avg'];
...
else if(isset($_GET['ringtone_list']))
{
	$jsonObj= array();
	...
	$query="SELECT * FROM tbl_ringtone ORDER BY id $gif_order";
	$sql = mysqli_query($mysqli,$query)or die(mysql_error());
...
else if(isset($_GET['ringtone_id']))
{
	...
	$query="SELECT * FROM tbl_ringtone WHERE id='".$_GET['ringtone_id']."'";
	$sql = mysqli_query($mysqli,$query)or die(mysqli_error());
	...
	$view_qry=mysqli_query($mysqli,"UPDATE tbl_ringtone SET total_views = total_views + 1 WHERE id = '".$_GET['ringtone_id']."'");
...
else if(isset($_GET['get_ringtone_wallpaper_most_viewed']))
{

	$limit=($_GET['page']-1) * 10;

	$query_rec = "SELECT COUNT(*) as num FROM tbl_ringtone
	ORDER BY tbl_ringtone.total_views";
	...
	$query="SELECT * FROM tbl_ringtone
	ORDER BY tbl_ringtone.total_views DESC LIMIT $limit,10";
...
else if(isset($_GET['get_ringtone_wallpaper_most_rated']))
{

	$limit=($_GET['page']-1) * 10;

	$query_rec = "SELECT COUNT(*) as num FROM tbl_ringtone
	ORDER BY tbl_ringtone.total_rate";
	...
	$query="SELECT * FROM tbl_ringtone
	ORDER BY tbl_ringtone.total_rate DESC LIMIT $limit,10";
...
else if(isset($_GET['ringtone_search_text']))
{

	$jsonObj= array();	

	$query="SELECT * FROM tbl_ringtone
	WHERE ringtone_tag LIKE '%".$_GET['ringtone_search_text']."%' ORDER BY tbl_ringtone.ringtone_tag";
...
else if(isset($_GET['ringtone_rate']))
{
	$jsonObj= array();	
	...
	$query1 = mysqli_query($mysqli,"select * from tbl_rating_ringtone where post_id  = '$post_id' && user_id = '$user_id'"); 
	      //exit;
	while($data1 = mysqli_fetch_assoc($query1)){
		$rate_db1[] = $data1;
	}
	...
	if(@count($rate_db1) == 0 ){
		...
		$res_rat=mysqli_query($mysqli, "INSERT INTO tbl_rating_ringtone(`post_id`,`user_id`,`rate`,`ip`) VALUES ('$post_id','$user_id','$therate','$ip')"); 

	  		//$qry = Insert('tbl_rating',$data_r); 

	          //Total rate result

		$query = mysqli_query($mysqli,"select * from tbl_rating_ringtone where post_id  = '$post_id' ");
...
else if(isset($_GET['get_ringtone_rate']))
{
...
else if(isset($_GET['ringtone_download_id']))
	{
		...
		$view_qry=mysqli_query($mysqli,"UPDATE tbl_ringtone SET total_download = total_download + 1 WHERE id = '".$_GET['ringtone_download_id']."'");
 	 

    	$total_dw_sql="SELECT * FROM tbl_ringtone WHERE id='".$_GET['ringtone_download_id']."'";
	    $total_dw_res=mysqli_query($mysqli,$total_dw_sql);
	    $total_dw_row=mysqli_fetch_assoc($total_dw_res);
